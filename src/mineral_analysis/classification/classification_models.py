...
class SpectralLSTM(nn.Module):
    def __init__(self, input_size=1, hidden_size=128, num_layers=2, num_classes=4, dropout=0.3):
    ...
    def forward(self, x):
        # x: (B, 1, L) -> need (B, L, 1)
        x = x.permute(0, 2, 1)
        out, _ = self.lstm(x)        # out: (B, L, hidden*2)
        out = out[:, -1, :]          # take last timestep
        out = self.fc(out)
        return out
class SpectralGRU(nn.Module):
    def __init__(self, input_size=1, hidden_size=128, num_layers=2, num_classes=4, dropout=0.3):
        super(SpectralGRU, self).__init__()
...
def load_model_and_predict(model_path: str, endmembers: np.ndarray) -> Tuple[np.ndarray, np.ndarray, Dict]:
    ...
    # Add channel dimension: (n_endmembers, n_bands) -> (n_endmembers, 1, n_bands)
    # SpectralCNN expects input shape (B, 1, L)
    endmembers_tensor = torch.FloatTensor(endmembers).unsqueeze(1).to(device)
    ...
    for i, (pred_class, probs) in enumerate(zip(predicted_classes, probabilities)):
        ...
        top4_indices = np.argsort(probs)[-4:][::-1]
        print(f"  Top 4: ", end="")
        for j, idx in enumerate(top4_indices):
...
if __name__ == "__main__":
    ...
    model_1dcnn = SpectralCNN(num_classes=num_classes).to(device)
    train_test_loop(model_1dcnn)
    
    print("\nTraining SpectralLSTM...")
    model_lstm = SpectralLSTM(num_classes=num_classes).to(device)
    train_test_loop(model_lstm)

    print("\nTraining SpectralGRU...")
    model_gru = SpectralGRU(num_classes=num_classes).to(device)
    train_test_loop(model_gru)